...
using Interpolations, LinearAlgebra, UnPack, DiffEqBase, SparseArrays
...
function make_model_builder(
    ...
    function model_builder(nx, Δx::T, p₀, coeff_df = nothing; bc = :periodic) where {T}
        ...
        U = rand(nu * nx)
        dU = zeros(nx * nu)
        FiniteDiff.finite_difference_jacobian!(J, (dU, U) -> update!(dU, U, p₀, 0.0), U)
        sparsity = sparse(J)
        colorvec = matrix_colors(sparsity)
        sparsecache = FiniteDiff.JacobianCache(U, colorvec = colorvec, sparsity = sparsity)
        jac!(J, U, p, t) = FiniteDiff.finite_difference_jacobian!(J, (dU, U) -> update!(dU, U, p, 0.0), U, sparsecache)

        return ODEFunction(update!, jac = jac!, jac_prototype = J, colorvec = colorvec), ints
    end
    return model_builder
end

function model_onesided_2eq(dU, U, p, t, c, ops::NamedTuple, ints::NamedTuple; openflow = false)
    nu = 2
...
function model_onesided_3eq(dU, U, p, t, c, ops::NamedTuple, ints::NamedTuple; openflow = false)
    nu = 3
    @unpack Dx, Dxx, Dxxx = ops
    @unpack h, qₗ, qₚ = c
...
function model_twosided(dU, U, p, t, c, ops::NamedTuple, ints::NamedTuple; openflow = false)
    nu = 5
    @unpack Dx, Dxx, Dxxx = ops
